#include <iostream>
#include <iomanip>
#include <string>
using namespace std;

int main()
{
    int orderEntry;
    int seat;
    int paymentMethod;
    double price;
    int quantity;
    double orderTotal = 0.0;
    double tax = 0.0;
    double total = 0.0;
    char addOn;

    cout << "                                        ===================================" << endl;
    cout << "                                        |                                 |" << endl;
    cout << "                                        |          OOOOO    SSSSS         |" << endl;
    cout << "                                        |         O     O  S              |" << endl;
    cout << "                                        |         O     O   SSSSS         |" << endl;
    cout << "                                        |         O     O        S        |" << endl;
    cout << "                                        |          OOOOO    SSSSS         |" << endl;
    cout << "                                        |                                 |" << endl;
    cout << "                                        |       ORDERING SYSTEM 1.0       |" << endl;
    cout << "                                        |_________________________________|" << endl << endl;

    cout <<  "1. Order Entry" << endl;
    cout << "2. Reporting" << endl;
    cout << "3. Exit" << endl << endl;

    while (true) {
        cout << "Please enter Your Category (1-3): ";
        cin >> orderEntry;

        if (orderEntry == 1) {
            cout << endl << "--- Price List ---" << endl;
            cout << "1. ROCKZONE:    RM 149.00" << endl;
            cout << "2. VIP:         RM 1299.00" << endl;
            cout << "3. PLATINUM:    RM 649.00" << endl;
            cout << "4. GOLD ZONE:   RM 219.00" << endl;
            cout << "5. SILVER ZONE: RM 99.00" << endl;
            cout << "----------------------------" << endl << endl;

            while (true) {
                cout << "Please select your seat (1 ~ 5): ";
                cin >> seat;

                switch (seat) {
                case 1:
                    price = 149.0;
                    cout << "Selected: ROCKZONE" << endl; 
                    break;
                case 2:
                    price = 1299.0;
                    cout << "Selected: VIP" << endl; 
                    break;
                case 3:
                    price = 649.0;
                    cout << "Selected: PLATINUM" << endl; 
                    break;
                case 4:
                    price = 219.0;
                    cout << "Selected: GOLD ZONE" << endl; 
                    break;
                case 5:
                    price = 99.0;
                    cout << "Selected: SILVER ZONE" << endl; 
                    break;
                default:
                    cout << "Invalid seat selected. Please enter 1-5." << endl;
                    continue;
                }

                cout << "Please enter quantities: ";
                cin >> quantity;
                while (quantity <= 0) {
                    cout << "Invalid quantity. Please enter a positive number." << endl;
                    cout << "Please enter quantities: ";
                    cin >> quantity;
                }

                double itemTotal = quantity * price;
                orderTotal += itemTotal;

                cout << fixed << setprecision(2);
                cout <<endl << "Current item total: RM " << itemTotal << endl;
                cout << "Current order total: RM " << orderTotal << endl << endl;

                do {
                    cout << "Do you want to add more tickets? (Y/N) > ";
                    cin >> addOn;
                    addOn = toupper(addOn);
                    if (addOn != 'Y' && addOn != 'N') {
                        cout << "Invalid input. Please enter Y or N." << endl;
                    }
                } while (addOn != 'Y' && addOn != 'N');

                if (addOn == 'N') {
                    
                    tax = orderTotal * 0.06;
                    total = orderTotal + tax;

                    cout << endl << "Proceeding to cart." << endl;
                    cout << "------------------------------------------" << endl;
                    cout << left << setw(30) << "Subtotal:" << right << "RM " << setw(8) << orderTotal << endl;
                    cout << left << setw(30) << "6% Tax:" << right << "RM " << setw(8) << tax << endl;
                    cout << "------------------------------------------" << endl;
                    cout << left << setw(30) << "Total:" << right << "RM " << setw(8) << total << endl << endl;

                    while (true) {
                        cout << "Please select your payment method:" << endl;
                        cout << "1. Credit Card" << endl;
                        cout << "2. Online Banking" << endl;
                        cout << "3. E-Wallet" << endl << endl;
                        cout << "Enter payment method (1-3): ";
                        cin >> paymentMethod;

                        if (paymentMethod >= 1 && paymentMethod <= 3) 
                            break;
                        cout << "Invalid input. Please enter 1-3." << endl;
                    }

                    switch (paymentMethod) {
                    case 1:
                        cout << "You selected: Credit Card" << endl;
                        break;
                    case 2:
                        cout << "You selected: Online Banking" << endl;
                        break;
                    case 3:
                        cout << "You selected: E-Wallet" << endl;
                        break;
                    }

                    cout << "Payment successful! Thank you for your purchase." << endl << endl;
                    break; 
                 }              
              }
        }
        else if (orderEntry == 2) {
            if (orderTotal > 0) {
                cout << endl << "-------------- Order Report --------------" << endl;
                cout << left << setw(15) << "Seat Type" << right << setw(10) << "Quantity" << right << setw(15) << "Item Total" << endl;

                cout << "------------------------------------------" << endl;

            

                cout << "------------------------------------------" << endl;
                cout << left << setw(30) << "Subtotal:" << right << "RM " << setw(8) << orderTotal << endl;
                cout << left << setw(30) << "6% Tax:" << right << "RM " << setw(8) << tax << endl;
                cout << "------------------------------------------" << endl;
                cout << left << setw(30) << "Total:" << right << "RM " << setw(8) << total << endl << endl;
            }
            else {
                cout << endl << "No order placed yet." << endl << endl;
            }
        }

        else if (orderEntry == 3) {
            cout << "Exiting program. Thank you!" << endl;
            break;
        }

        else {
            cout << "Invalid input. Please enter 1-3." << endl;
        }
    }

    return 0;
}